            ...
            r.on :year do |year|
                if YEARS.member?(year)
                    r.is do
                        ...
                        ret[:mps] = MP.filter(:year => year).order_by(:state_or_ut, :constituency).map{|mp| mp.format}
                        ret
                    end
                    ...
                    r.get :state do |state|
                        if MP_STATES.member?(state)
                            ...
                            ret[:mps] = MP.filter(:year => year, :state_or_ut => state).order_by(:constituency).map{|mp| mp.format}
                        else
